...
data "coder_parameter" "doppler_token" {
  ...
  description  = "The Doppler auth token for accessing YodaSpeak config and secrets"
  default      = ""
  type         = "string"
...
resource "coder_agent" "main" {
  ...
  startup_script = <<-EOT
    ...
    if [ ! -d "$REPO_PATH" ]; then
      ...
      printf "\n[info]: Installing dev and production dependencies\n"
      cd "$REPO_PATH"
  ...
  spec {
    ...
    template {
      ...
      spec {
        ...
        container {
          name              = "dev"
          image             = "ryanblunden/yodaspeak-coder:0.1.0"
